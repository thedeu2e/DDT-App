...
def generate_elephant_flows(id, duration, net):
    ...
    send = hosts[:middle_index]
    receive = hosts[middle_index:]

    # select random src and dst
    s = random.sample(send, 1)
    r = random.sample(receive,1)
    src = net.get(str(s[0]))
    dst = net.get(str(r[0]))
...
def generate_mice_flows(id, duration, net):
